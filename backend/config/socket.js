...
// Socket.io bypasses the Express middleware chain, so the
// sanitizeXss middleware (which escapes req.body strings at HTTP
// ingress) never runs for socket events. Without this, any string
// a client emits via socket (message content, image URLs, etc.) is
// persisted raw to the DB and broadcast back to every participant —
    ...
    socket.on('send_message', async data => {
      try {
        ...
        const message = await db('messages').create({
          ...
          receiver: receiverId,
          // Escape user-supplied strings before persisting — Socket.io
          // bypasses express middleware, so without this an attacker can
          // store <script> payloads via socket.emit and have them render
          // via innerHTML on every participant's chat view.
          content: sanitizeSocketString(content),
          type,
          imageUrl: type === 'image' ? sanitizeSocketString(imageUrl) : undefined,
