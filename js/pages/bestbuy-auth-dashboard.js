...
(function () {
  ...
  Pages.renderRegister = async function () {
    ...
    var uniOptions = config.universities.map(function (u) {
  ...
  Pages.handleRegisterBB = async function (event) {
    ...
    var result = await authManager.register(fullName, email, password, university);
    if (result.success) {
      Pages.updateNavbar();
      Pages.updateCartBadge();
      Pages.renderDashboard();
  ...
  Pages.renderDashboard = function () {
    var currentUser = StorageManager.get(STORAGE_KEYS.CURRENT_USER, true);
    if (!currentUser) {
    ...
    var orders = checkoutManager.getUserOrders(currentUser.id || '');
    var wishlist = productsManager.getWishlist();
    var cartCount = cartManager.getCount();
    var initials = currentUser.fullName.split(' ').map(function (n) { return n[0]; }).join('').toUpperCase().slice(0, 2);
      ...
      '<div class="bb-dashboard">' +
          ...
          '<div class="bb-dashboard-header-inner">' +
            ...
            '<div class="bb-dashboard-user">' +
              '<div class="bb-dashboard-info">' +
                '<div class="bb-dashboard-user-name">' + currentUser.fullName + '</div>' +
                '<div class="bb-dashboard-user-email">' + currentUser.email + '</div>' +
              '</div>' +
              '<div class="bb-dashboard-avatar">' + initials + '</div>' +
            '</div>' +
          '</div>' +
        '</div>' +
        '<div class="bb-dashboard-layout">' +
          '<nav class="bb-dashboard-nav">' +
            ...
            '<div class="bb-dashboard-nav-item"><button class="bb-dashboard-nav-link" data-tab="orders" onclick="Pages.switchDashboardTabBB(\'orders\')"><span class="bb-dashboard-nav-icon">📦</span>Orders' + (orders.length > 0 ? '<span class="bb-dashboard-nav-badge">' + orders.length + '</span>' : '') + '</button></div>' +
            '<div class="bb-dashboard-nav-item"><button class="bb-dashboard-nav-link" data-tab="wishlist" onclick="Pages.switchDashboardTabBB(\'wishlist\')"><span class="bb-dashboard-nav-icon">❤️</span>Wishlist' + (wishlist.length > 0 ? '<span class="bb-dashboard-nav-badge">' + wishlist.length + '</span>' : '') + '</button></div>' +
            '<div class="bb-dashboard-nav-item"><button class="bb-dashboard-nav-link" data-tab="cart" onclick="Pages.switchDashboardTabBB(\'cart\')"><span class="bb-dashboard-nav-icon">🛒</span>Cart' + (cartCount > 0 ? '<span class="bb-dashboard-nav-badge">' + cartCount + '</span>' : '') + '</button></div>' +
          ...
          '<main class="bb-dashboard-main">' +
            ...
            '<div class="bb-panel active" id="bb-panel-overview">' +
              '<div class="bb-panel-header">' +
                '<h2 class="bb-panel-title">Welcome back, ' + currentUser.fullName.split(' ')[0] + '!</h2>' +
                '<p class="bb-panel-subtitle">Here\'s what\'s happening with your account today.</p>' +
              '</div>' +
              '<div class="bb-stats-row">' +
                '<div class="bb-stat-card"><div class="bb-stat-value">' + orders.length + '</div><div class="bb-stat-label">Total Orders</div></div>' +
                '<div class="bb-stat-card"><div class="bb-stat-value">' + wishlist.length + '</div><div class="bb-stat-label">Wishlist Items</div></div>' +
                '<div class="bb-stat-card"><div class="bb-stat-value">' + cartCount + '</div><div class="bb-stat-label">Cart Items</div></div>' +
                '<div class="bb-stat-card"><div class="bb-stat-value" style="font-size:1rem;">' + (currentUser.university ? currentUser.university.toUpperCase() : 'N/A') + '</div><div class="bb-stat-label">University</div></div>' +
              '</div>' +
              ...
              '<div class="bb-orders-list">' +
                (recentOrders.length > 0 ? recentOrders.map(function (order) {
                  return '<div class="bb-order-card">' +
                    '<div class="bb-order-icon">📦</div>' +
                    '<div class="bb-order-info"><div class="bb-order-number">Order #' + order.orderNumber + '</div><div class="bb-order-details">' + order.items.length + ' item(s) • ' + Formatter.formatPrice(order.pricing.grandTotal) + '</div></div>' +
                    '<div class="bb-order-amount">' + Formatter.formatTimeAgo(order.createdAt) + '</div>' +
                    '<span class="bb-order-status ' + (order.status ? order.status.toLowerCase() : 'placed') + '">' + (order.status ? order.status.charAt(0).toUpperCase() + order.status.slice(1) : 'Placed') + '</span>' +
                    '</div>';
                }).join('') :
            ...
            '<div class="bb-panel" id="bb-panel-wishlist">' +
              ...
              '</div>' +
              (wishlist.length > 0 ?
                '<div class="bb-wishlist-grid">' + wishlist.map(function (product) {
                  return '<div class="bb-wishlist-card" onclick="Pages.renderProductDetail(\'' + product.id + '\')">' +
                    '<img src="' + product.images[0] + '" alt="' + product.title + '" class="bb-wishlist-image" />' +
                    '<div class="bb-wishlist-info">' +
                      '<h4 class="bb-wishlist-title">' + product.title + '</h4>' +
                      '<div class="bb-wishlist-price">' + product.price.toLocaleString() + ' GHS</div>' +
                    '</div>' +
                    '</div>';
                }).join('') + '</div>' :
            ...
            '<div class="bb-panel" id="bb-panel-cart">' +
              ...
              '</div>' +
              (cartCount > 0 ?
                '<div class="bb-orders-list">' + cartManager.getItems().map(function (item) {
                  return '<div class="bb-order-card">' +
                    '<div class="bb-order-icon">🛒</div>' +
                    '<div class="bb-order-info"><div class="bb-order-number">' + item.product.title + '</div><div class="bb-order-details">Qty: ' + item.quantity + ' × ' + item.product.price.toLocaleString() + ' GHS</div></div>' +
                    '</div>';
                }).join('') + '</div>' +
            ...
            '<div class="bb-panel" id="bb-panel-profile">' +
              ...
              '<div class="bb-profile-form">' +
                '<form id="profile-form-bb" onsubmit="Pages.handleProfileUpdate(event)">' +
                  '<div class="bb-form-group"><label class="bb-form-label">Full Name</label><input type="text" id="fullName" name="fullName" class="bb-form-input" value="' + currentUser.fullName + '" required /></div>' +
                  '<div class="bb-form-group"><label class="bb-form-label">Email</label><input type="email" id="email" name="email" class="bb-form-input" value="' + currentUser.email + '" required /></div>' +
                  '<div class="bb-form-group"><label class="bb-form-label">Phone</label><input type="tel" id="phone" name="phone" class="bb-form-input" value="' + (currentUser.phone || '') + '" /></div>' +
                  '<div class="bb-form-group"><label class="bb-form-label">University</label><input type="text" class="bb-form-input" value="' + (currentUser.university || 'Not set') + '" disabled /></div>' +
            ...
            '<div class="bb-panel" id="bb-panel-settings">' +
              ...
              '</div>' +
              '<div class="bb-profile-form">' +
                '<div class="bb-form-group"><label class="bb-form-label">Account Status</label><input type="text" class="bb-form-input" value="' + (currentUser.isVerified ? '✓ Verified' : '⏳ Pending Verification') + '" disabled /></div>' +
                '<div class="bb-form-group"><label class="bb-form-label">Role</label><input type="text" class="bb-form-input" value="' + (currentUser.role ? currentUser.role.charAt(0).toUpperCase() + currentUser.role.slice(1) : 'Buyer') + '" disabled /></div>' +
  ...
  Pages.updateNavbar = function () {
    if (originalUpdateNavbar) originalUpdateNavbar.call(this);
    var currentUser = StorageManager.get(STORAGE_KEYS.CURRENT_USER, true);
