...
class API {
  constructor(baseURL = null) {
    ...
    try {
      ...
      /* VITE_API_URL only available in Vite dev */
    }
    this.baseURL =
      baseURL ||
  ...
  get isStaticDeploy() {
    if (this._isStaticDeploy) {
      return true;
    }
    if (this._backendProbed && this._backendReachable === false) {
      return true;
    }
    return false;
  }
  ...
  _readSession(storageKey) {
    try {
      ...
      /* malformed session — treat as absent */
    }
    return null;
  }
  ...
  getTokenFor(url) {
    ...
    if (isAdminUrl || inAdminPanel) {
      ...
      return (
        this._readSession('unihub_admin_session')?.token ??
        this._readSession('unihub_session')?.token ??
        null
      );
    }
  ...
  _compressImage(file, maxDim = 1200, quality = 0.7) {
    return new Promise(resolve => {
      ...
      const isHeic =
        heicExts.some(ext => file.name.toLowerCase().endsWith(ext)) ||
      ...
      if (compressed.size > 9 * 1024 * 1024) {
        throw new Error(
          `Image too large (${(compressed.size / 1024 / 1024).toFixed(1)}MB) — max 9MB. Try a smaller photo.`
        );
      }
      return compressed;
    });
    ...
    images: async files => {
      ...
      try {
        ...
        if (first.error) {
          ...
          if (retry.error) {
            return { success: false, error: retry.error, urls: [], failures: [] };
          }
          if (retry.response) {
        ...
        if (!first.response) {
          return { success: false, error: first.error || 'Upload failed', urls: [], failures: [] };
        }
          ...
          if (retryCsrf) {
            ...
            if (retry.response) {
              ...
              if (retryData.success && retryData.urls && retryData.urls.length > 0) {
                return { success: true, urls: retryData.urls, failures: retryData.failures || [] };
              }
              return {
                success: false,
                error: retryData.error || retryData.message || 'Upload failed',
                urls: [],
                failures: [],
              };
            }
        ...
        return {
          success: false,
          error: data.error || data.message || `Upload failed (HTTP ${first.response.status})`,
          urls: [],
          failures: [],
        };
      } catch (err) {
