...
class AdminReportsManager {
  ...
  async _getOrders() {
    ...
    if (!Array.isArray(this._ordersCache)) {
      this._ordersCache = [];
    }
    this._cacheTime = now;
    return this._ordersCache;
  ...
  async getDashboardOverview() {
    ...
    const activeOrders = orders.filter(o =>
      ['placed', 'confirmed', 'in_transit'].includes(o.status)
    ).length;

    let pendingVerifications = 0;
    try {
      pendingVerifications = adminVerificationsManager.getStats().pending;
    } catch (e) {
      console.warn('reports: getStats failed:', e);
    }
    ...
    return {
      ...
      today: {
        ...
        newUsers: users.filter(u => new Date(u.joinedDate || u.createdAt).toDateString() === today)
          .length,
      },
      thisWeek: {
  ...
  async _buildReportHTML(type, options) {
    ...
    try {
      if (type === 'sales') {
        ...
        const users = adminUsersManager.getAllUsers();
        content +=
          '<h2>User Summary</h2><table><tr><th>ID</th><th>Name</th><th>Email</th><th>University</th><th>Role</th><th>Verified</th></tr>';
        users.forEach(u => {
