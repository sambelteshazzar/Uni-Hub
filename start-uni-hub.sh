...
if command -v mongosh &> /dev/null; then
    if mongosh --eval "db.adminCommand('ping')" > /dev/null 2>&1; then
        ...
        echo -e "${YELLOW}⚠ MongoDB connection failed. Make sure it's running!${NC}"
        echo -e "${YELLOW}Start MongoDB with: mongod or docker run -d -p 27017:27017 mongo${NC}"
    ...
    echo -e "${YELLOW}⚠ MongoDB tools not found. Verify MongoDB is running on localhost:27017${NC}"
fi
...
echo "  Admin Email: admin@unihub.local"
