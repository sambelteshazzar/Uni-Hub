# Privacy Policy & Terms Rewrite Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace generic /privacy and /terms templates with Act-843-aligned, version-stamped content from a single source module, and fix dead signup-consent links — per `docs/superpowers/specs/2026-08-23-policies-rewrite-design.md`.

**Architecture:** New hybrid-module `js/content/policies.js` (`export const POLICIES` + `window.POLICIES`, matching repo conventions) holds VERSION/LAST_UPDATED and both documents as `{heading, body}` section arrays. `StaticPageMethods.renderPrivacy/renderTerms` iterate them. Registered in `js/app-init.js`'s module list before `static-pages`. Signup checkbox links point at the real routes.

**Tech Stack:** Vanilla JS SPA, no build deps.

## Global Constraints

- NO new npm dependencies.
- `POLICIES.VERSION` MUST be `'2026-08-1'` — identical string to `backend/config/policies.js` POLICY_VERSION (consent records store it).
- Include the in-code comment: drafts are not legal advice; obtain Ghanaian lawyer review (AGENTS.md standards-hook).
- No NEW inline event handlers; escape nothing here (content is static authored text, not user data).
- Backend untouched. Gates: eslint on touched files vs recorded baselines; `npm run build`; existing Playwright admin specs stay green.
...
|---|---|---|
| `js/content/policies.js` | Create | POLICIES: VERSION, LAST_UPDATED, privacy[], terms[] |
| `js/app-init.js` | Modify | Register content module before static-pages |
| `js/pages/static-pages.js` | Modify | Renderers consume POLICIES arrays |
| `js/pages/auth-pages.js` | Modify | Dead `#` links → `#/terms`, `#/privacy` |

---

### Task 1: Content module + renderers + links

**Files:**
- Create: `js/content/policies.js`
- Modify: `js/app-init.js`, `js/pages/static-pages.js`, `js/pages/auth-pages.js`

**Interfaces:**
- Produces: global `POLICIES = { VERSION, LAST_UPDATED, privacy: [{heading, body}...], terms: [...] }`. Renderers consume `.privacy` / `.terms`.

- [ ] **Step 1: Create `js/content/policies.js`**
...
NOTE on the `${'${'}POLICIES.VERSION}` sequences above: inside this plan they are written so the FINAL file contains the template literal `${POLICIES.VERSION}` (JS interpolation at page-render time), NOT literal text. When writing `js/content/policies.js` the terms headings/body strings must be BACKTICK template literals containing real `${POLICIES.VERSION}` interpolations.

- [ ] **Step 2: Register the module in `js/app-init.js`**

In the `pages:` array of MODULE_GROUPS, insert BEFORE the `static-pages` entry:

```js
    { name: 'policies-content', file: 'js/content/policies.js', exposes: ['POLICIES'] },
```

- [ ] **Step 3: Consume in `js/pages/static-pages.js`**

Replace the hardcoded section HTML inside `renderTerms()` with:
...
And the version line at top becomes:

```js
        <p style="color: var(--text-secondary); margin-bottom: 2rem;">Version ${POLICIES.VERSION} · Last updated: ${POLICIES.LAST_UPDATED}</p>
```

Apply the identical transformation in `renderPrivacy()` using `POLICIES.privacy`. Keep both functions' outer shell (container div, h1, showOriginalNavFooter, scrollTo) intact. Since static-pages.js has no imports, reference the global `POLICIES` directly (exposed via window in Step 1).

- [ ] **Step 4: Fix signup consent links**

In js/pages/auth-pages.js (~line 1069), replace:
...
Run: `npx eslint js/content/policies.js js/pages/static-pages.js js/pages/auth-pages.js && npm run build`
Expected: eslint counts ≤ per-file baselines (record before editing); build succeeds.

- [ ] **Step 6: Commit**

```bash
git add js/content/policies.js js/app-init.js js/pages/static-pages.js js/pages/auth-pages.js
git commit -m "Publish versioned privacy policy and terms with working consent links"
```

---

### Task 2: Verification chain

- [ ] **Step 1:** `npm run lint:check && npm run build`
  Expected: root lint at its baseline (1426±1); build succeeds.
- [ ] **Step 2:** `npx playwright test e2e/admin.spec.js`
  Expected: green (regression check that loader changes didn't break module bootstrapping).
- [ ] **Step 3:** Commit any fixes found:
```bash
git add -A && git commit -m "Fix issues found during policies verification"
```
...
1. Visit `/#/terms` and `/#/privacy` → all sections render, header shows "Version 2026-08-1 · Last updated: August 2026".
2. Signup screen → both checkbox links navigate to the correct pages.
3. Backend cross-check: `node -e "console.log(require('./backend/config/policies').POLICY_VERSION)"` prints `2026-08-1`, matching the page.
4. Register a fresh test account → consent row's policyVersion matches what the pages display.
